...
const OperationReviewAuditAppealReview = "/api.review.v1.Review/AuditAppealReview"
...
func RegisterReviewHTTPServer(s *http.Server, srv ReviewHTTPServer) {
	...
	r.POST("/v1/review/auditAppeal", _Review_AuditAppealReview0_HTTP_Handler(srv))
}
...
func _Review_AuditAppealReview0_HTTP_Handler(srv ReviewHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in AuditAppealReviewRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		if err := ctx.BindQuery(&in); err != nil {
		...
		http.SetOperation(ctx, OperationReviewAuditAppealReview)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.AuditAppealReview(ctx, req.(*AuditAppealReviewRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*AuditAppealReviewReply)
		return ctx.Result(200, reply)
	}
}
...
type ReviewHTTPClient interface {
	...
	// AuditAppealReview	O端审核评价申诉
	AuditAppealReview(ctx context.Context, req *AuditAppealReviewRequest, opts ...http.CallOption) (rsp *AuditAppealReviewReply, err error)
	// AuditReview	O端审核评价
...
// AuditAppealReview	O端审核评价申诉
func (c *ReviewHTTPClientImpl) AuditAppealReview(ctx context.Context, in *AuditAppealReviewRequest, opts ...http.CallOption) (*AuditAppealReviewReply, error) {
	var out AuditAppealReviewReply
	pattern := "/v1/review/auditAppeal"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationReviewAuditAppealReview))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
